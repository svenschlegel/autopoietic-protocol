...
- [5. Governance & Routing Physics](#5-governance--routing-physics)
  ...
  - [5.2 Mass Accrual & Soulbound Identity](#52-mass-accrual--soulbound-identity)
...
### 5.2 Mass Accrual & Soulbound Identity

Soulbound Mass is the non-transferable reputation token that determines an agent's position in the Phase Space. Mass is accrued via `M_new = M_old + (Bounty × σ)`, where σ is the efficiency coefficient (`T_network_avg / T_agent`, clamped to [0.1, 3.0]). Faster agents earn more Mass per payload. Mass cannot be bought, sold, or delegated — it can only be earned through verified metabolic work.
...
### A.5 Soulbound Mass Accrual (σ)

```
σ = T_network_avg / T_agent
```

Domain: σ ∈ [0.1, 3.0] (clamped). T_agent: wall-clock solve time. T_network_avg: rolling 30-day average for equivalent payloads. Mass update: `M_new = M_old + (Bounty × σ)`.
...
Protocol specification V3.4 with GPSL integration and Cryptographic Lock. Agent-based simulation validating all 6 core predictions (routing distribution, malicious pruning at 86%, anti-monopoly, organic mode shifts, moderate inequality at Gini 0.583, $395k simulated USDC flow). Four Solidity smart contracts deployed on Base Sepolia (SoulboundMass, EscrowCore, AutoToken, Treasury). 116 Foundry tests passing (100%). First Metabolic Payload successfully solved on-chain (Tier 1 verification, USDC payout, Mass accrual). Node Client prototype with Web3 chain adapter. GitHub repository published.
...
**Phase 3 (Current):** Multi-node competitive routing tests, editorial polish of whitepaper, Base Builder Grant application.

**Phase 4:** Competitive smart contract audit via Code4rena or Sherlock ($10–15k). DUNA formation via OtoCo.

**Phase 5:** Mainnet deployment on Base. Genesis Sequence execution. Go rewrite of Node Client networking layer.
...
| **Soulbound Mass** | Non-transferable reputation earned through verified work. Determines routing priority. |
